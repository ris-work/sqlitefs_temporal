...
impl Sqlite {
    ...
    pub fn new_at_time(path: &Path, time: String) -> Result<Self> {
        ...
        //conn.execute("CREATE TEMP TABLE txattr_audit_entries AS SELECT * FROM xattr_audit WHERE timestamp_utc < (?1);", params![time] )?;
        conn.execute("CREATE TEMP TABLE tdentry AS SELECT * FROM (SELECT max_seq, latest.parent_id, latest.child_id, TG_OP, latest.name, tdentry_audit_entries.file_type from (SELECT max(seq) as max_seq, parent_id, child_id, name FROM tdentry_audit_entries as latest GROUP BY parent_id, child_id, name) as latest INNER JOIN tdentry_audit_entries ON tdentry_audit_entries.seq=max_seq AND tdentry_audit_entries.child_id=latest.child_id AND tdentry_audit_entries.name = latest.name AND tdentry_audit_entries.parent_id=latest.parent_id) WHERE TG_OP IS NOT 'DELETE';", [])?;
        conn.execute("CREATE TEMP TABLE tdata AS SELECT * FROM (SELECT max_seq, latest.block_num, latest.file_id, tdata_audit_entries.data, TG_OP from (SELECT max(seq) as max_seq, file_id, block_num FROM tdata_audit_entries as latest GROUP BY file_id, block_num) as latest INNER JOIN tdata_audit_entries ON tdata_audit_entries.seq=max_seq AND tdata_audit_entries.file_id=latest.file_id AND tdata_audit_entries.block_num = latest.block_num) WHERE TG_OP IS NOT 'DELETE';", [])?;
        conn.execute("CREATE TEMP TABLE txattr AS SELECT * FROM (SELECT max_seq, latest.name, latest.file_id, txattr_audit_entries.name, txattr_audit_entries.value, TG_OP from (SELECT max(seq) as max_seq, file_id, name FROM txattr_audit_entries as latest GROUP BY file_id, name) as latest INNER JOIN txattr_audit_entries ON txattr_audit_entries.seq=max_seq AND txattr_audit_entries.file_id=latest.file_id AND txattr_audit_entries.name = latest.name) WHERE TG_OP IS NOT 'DELETE';", [])?;
        conn.execute("CREATE TEMP TABLE tmetadata AS SELECT * FROM (SELECT max_seq, tmetadata_audit_entries.id, size, atime, atime_nsec, mtime, mtime_nsec, ctime, ctime_nsec, crtime, crtime_nsec, kind, mode, nlink, uid, gid, rdev, flags, TG_OP from (SELECT max(seq) as max_seq, id FROM tmetadata_audit_entries as latest GROUP BY id) as latest INNER JOIN tmetadata_audit_entries ON tmetadata_audit_entries.seq=max_seq AND tmetadata_audit_entries.id=latest.id) WHERE TG_OP IS NOT 'DELETE';", [])?;
        Ok(Sqlite {
            conn,
            read_only,
            time_recording,
...
impl DbModule for Sqlite {
    fn init(&mut self) -> Result<()> {
            ...
            "SELECT count(name) FROM sqlite_master WHERE type='table' AND name=$1";
        {
            ...
            let row_count: u32 =
                self.conn
                    .query_row(table_search_sql, params!["metadata_audit"], |row| {
                        row.get(0)
                    })?;
            if row_count == 0 {
        ...
        }
        {
            let row_count: u32 =
                self.conn
                    .query_row(table_search_sql, params!["data_audit"], |row| row.get(0))?;
                let sql = "CREATE TEMP TABLE data(\
                    file_id int,\
                    block_num int,\
                    data blob,\
            ...
            if !self.read_only {
                ...
                let sql_audit_trigger_update = "\
                CREATE TEMP TRIGGER audit_update_data AFTER UPDATE on data \
                BEGIN \
                ...
                let sql_audit_trigger_insert = "\
                CREATE TEMP TRIGGER audit_insert_data AFTER INSERT on data \
                BEGIN \
